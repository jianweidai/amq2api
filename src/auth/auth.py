...
from src.auth.account_manager import get_random_account, update_account_tokens, update_refresh_status
...
async def refresh_account_token(account: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 400 and "invalid_grant" in error_text:
            ...
            # 获取当前账号信息
            from src.auth.account_manager import get_account
...
async def refresh_legacy_token() -> bool:
    ...
    Returns:
        bool: 刷新成功返回 True
    """
    from src.config import read_global_config, update_global_config
